document.addEventListener("DOMContentLoaded", async () => {
    ...
    // it as an int. Then creates a new dropdown item for each value the user has selected and binds it to the Weapons.ProductionCost property.
        ...
        for (let i = 0; i < selectedValue; i++) {
            ...
            div.innerHTML = `
              <div style="border: 1px solid grey; text-align: center; padding: 10px;">
                <label for="Weapon_ProductionCost_${i}__MaterialId">
                  Material ${i + 1}
                </label>
                <select
                  name="Weapon.ProductionCost[${i}].MaterialId"
                  class="form-control"
                ...
                <label for="Weapon_ProductionCost_${i}__Amount">
                  Amount
                </label>
                <input
                  type="number"
                  name="Weapon.ProductionCost[${i}].Amount"
                  class="form-control"
                />
              </div>
              <br/>
    ...
    // Pretty much the same as material costs but the list is populated from the cshtml.cs variable SpecialProperties
